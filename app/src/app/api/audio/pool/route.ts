...
import { AuthenticationError, ValidationError } from '@/lib/utils/error-handling';
  ...
  try {
    ...
    for (const file of files) {
      if (!validateAudioFile(file)) {
        throw new ValidationError(`Invalid file type or size: ${file.name}. Only MP3/WAV files up to 50MB are allowed.`);
      }
    }
